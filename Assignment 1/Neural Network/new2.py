...
# Training set
dataset             = [np.array([[float(cell) for cell in row[:-1]] for row in csv.reader(open('winequality-red.csv'))]), np.array([float(row[-1]) for row in csv.reader(open('winequality-red.csv'))])                        ]
...
settings            = {
    # Required settings
    "n_inputs"              : 11,       # Number of network input signals
...
Prediction Example
"""
prediction_set = [ Instance([7.4,0.7,0,1.9,0.076,11,34,0.9978,3.51,0.56,9.4]), Instance([7.4,0.66,0,1.8,0.075,13,40,0.9978,3.51,0.56,9.4]) ]
